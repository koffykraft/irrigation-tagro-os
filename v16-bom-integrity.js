...
function boot(){
  ...
  function apply(){
    ...
    const countText=`Active pipe objects: Main ${a.counts.main} · Submain ${a.counts.submain} · Lateral ${a.counts.lateral}`;if(counts&&counts.textContent!==countText)counts.textContent=countText;
    const issues=[];
    ...
    if(issues.length){
      const state=panel.querySelector('#tagroBomState');if(state){if(state.textContent!=='DRAFT')state.textContent='DRAFT';state.classList.remove('ready')}
      ...
      const html=`<b>Integrity check:</b> ${issues.join(' · ')}. Review/delete extra objects and accept design inputs before treating quantities as final.`;if(integrity&&integrity.innerHTML!==html)integrity.innerHTML=html;
    }else if(integrity)integrity.remove();
  }
