...
function boot(){
  ...
  const selectors=['.tagro-cad.show','.tagro-bom.show','#tagroIdPanel.show','.v16-design','.v16-design-card','.v16-design-row','.tagro-bom-table','.tagro-bom-pager','.tagro-bom-tools','.tagro-bom-card','.tagro-bom-network','.tagro-bom-foot','[class*="formula"]','[class*="chart"]','table','select','input','textarea','button','canvas','svg'];
  ...
  const nameOf=el=>el.id||String(el.className||el.tagName).slice(0,120);
  function audit(label='runtime'){
    ...
    for(const el of inspected){
      const r=rect(el),s=getComputedStyle(el),n=nameOf(el);
      if(!inside(r,w,h,2)){geometry-=8;findings.push({type:'viewport',severity:'high',element:n,message:'Visible element extends outside viewport'})}
      if(el.scrollWidth>el.clientWidth+2&&s.overflowX==='visible'){rendering-=4;findings.push({type:'overflow-x',severity:'medium',element:n,message:'Horizontal content may overflow'})}
      if(el.scrollHeight>el.clientHeight+2&&['hidden','clip'].includes(s.overflowY)){rendering-=5;findings.push({type:'clipped-content',severity:'medium',element:n,message:'Content height is clipped by its container'})}
      if((el.matches('select,input,textarea')||el.matches('[class*="formula"],[class*="chart"],canvas,svg'))&&r.width<70){rendering-=3;findings.push({type:'narrow-control',severity:'low',element:n,message:`Rendered width is only ${Math.round(r.width)} px`})}
    }
    for(let i=0;i<panels.length;i++)for(let j=i+1;j<panels.length;j++){const a=rect(panels[i]),b=rect(panels[j]),area=overlap(a,b);if(area>16){geometry-=18;interaction-=12;findings.push({type:'panel-overlap',severity:'high',element:`${nameOf(panels[i])} × ${nameOf(panels[j])}`,message:`Active panels overlap by ${Math.round(area)} px²`})}}
    for(const el of interactive){const r=rect(el),s=getComputedStyle(el),n=nameOf(el);if(r.width<28||r.height<28){interaction-=2;findings.push({type:'target-size',severity:'low',element:n,message:`Interactive target is ${Math.round(r.width)}×${Math.round(r.height)} px`})}if(s.pointerEvents==='none'&&!el.disabled){interaction-=5;findings.push({type:'blocked-control',severity:'medium',element:n,message:'Visible enabled control cannot receive pointer events'})}}
    const bom=document.querySelector('.tagro-bom.show');if(bom&&visible(bom)){const table=bom.querySelector('.tagro-bom-table'),pager=bom.querySelector('.tagro-bom-pager');if(table){const visibleRows=[...table.querySelectorAll('tbody tr')].filter(visible),allRows=table.querySelectorAll('tbody tr').length;if(visibleRows.length>10){rendering-=8;findings.push({type:'pagination',severity:'medium',element:'BOM table',message:'More than 10 BOM rows visible on one page'})}if(table.scrollWidth>bom.clientWidth+4){geometry-=8;findings.push({type:'bom-width',severity:'high',element:'BOM table',message:'BOM table wider than its panel'})}if(allRows>visibleRows.length&&!pager){rendering-=8;findings.push({type:'pagination-missing',severity:'high',element:'BOM table',message:'Rows are hidden but pagination controls are absent'})}if(pager&&!inside(rect(pager),w,h,2)){interaction-=7;findings.push({type:'pager-offscreen',severity:'high',element:'BOM pager',message:'Pagination controls are not fully inside the viewport'})}}}
